...
import { Megaphone, Pin, Calendar, Eye, Search, Loader2 } from 'lucide-react';
...
export default function PengumumanPage() {
  ...
  const filtered = data.filter(p => {
    ...
    if (selKat && p.kategori !== selKat) return false;
    return true;
  });
  ...
  return (
    <div className="space-y-7 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input placeholder="Cari pengumuman..." className="pl-9 h-9 text-sm" value={search} onChange={e => setSearch(e.target.value)} /></div>
        <div className="flex gap-1.5 flex-wrap">
          <Button variant={selKat === null ? 'default' : 'outline'} size="sm" className={`text-xs h-9 btn-press ${selKat === null ? 'bg-primary' : ''}`} onClick={() => setSelKat(null)}>Semua</Button>
          {[...new Set(data.map(p => p.kategori))].map(k => (
            <Button key={k} variant={selKat === k ? 'default' : 'outline'} size="sm" className={`text-xs h-9 btn-press ${selKat === k ? 'bg-primary' : ''}`} onClick={() => setSelKat(selKat === k ? null : k)}>
              {KAT[k] || k}
            </Button>
          ))}
        </div>
      ...
      <div className="space-y-2 stagger-children">
        {filtered.length === 0 && <div className="empty-state"><Megaphone className="w-12 h-12 mx-auto empty-state-icon" /><h3>Tidak ada pengumuman</h3><p>Pengumuman akan muncul di sini setelah diterbitkan</p></div>}
        {filtered.map(p => {
